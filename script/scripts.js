...
function renderJobs(tab = 'all') {
    ...
    // Check if filtered jobs is empty
    if (filtered.length === 0) {
        // Show empty state
        ...
        let message = '';
        if (tab === 'all') message = 'No jobs available';
        else if (tab === 'interview') message = 'No interview jobs found';
        else if (tab === 'rejected') message = 'No rejected jobs found';
...
// Force empty state to be hidden initially if jobs exist
document.addEventListener('DOMContentLoaded', function () {
    // Initial render
    renderJobs('all');

    // Additional check to ensure empty state is properly hidden
    if (jobs.length > 0 && emptyState) {
        emptyState.style.display = 'none';
        emptyState.classList.add('hidden');
    }
});
